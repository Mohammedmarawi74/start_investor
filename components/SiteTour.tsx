...
const SiteTour: React.FC<SiteTourProps> = ({ onComplete, onSkip }) => {
  ...
  if (isFinished) {
    return (
      <div className="fixed inset-0 z-[10000] flex items-center justify-center p-6 bg-slate-950/20 backdrop-blur-sm transition-all duration-1000">
        <div className="bg-white rounded-[3rem] p-12 max-w-xl w-full text-center shadow-[0_50px_100px_rgba(0,0,0,0.2)] relative overflow-hidden border border-white">
          <div className="relative z-10">
            <div className="w-20 h-20 bg-emerald-100 text-emerald-600 rounded-[2.5rem] flex items-center justify-center mx-auto mb-8 shadow-xl">
              <CheckCircle2 size={40} strokeWidth={2.5} />
            </div>
            <h2 className="text-3xl font-black text-slate-900 mb-4 animate-bounce">تهانينا! أنت جاهز للإنطلاق</h2>
            ...
            <button 
              onClick={onComplete}
              className="w-full py-5 bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-2xl text-[15px] font-black shadow-2xl hover:scale-[1.02] active:scale-95 transition-all outline-none"
            >
              ابدأ الاستخدام الآن
            </button>
          </div>
  ...
  return (
    <div className="fixed inset-0 z-[10000] pointer-events-none">
      ...
      <div 
        ref={tooltipRef}
        className="absolute z-[10002] pointer-events-auto transition-all duration-500"
        style={{
      ...
      >
        <div className="bg-white rounded-[2.5rem] p-8 w-[350px] shadow-[0_30px_90px_rgba(0,0,0,0.18)] border border-slate-100 flex flex-col relative overflow-hidden backdrop-blur-xl">
          {/* Decorative elements */}
          ...
          <div className="flex items-center justify-between mt-auto pt-6 border-t border-slate-50 relative z-10">
            ...
            <div className="flex items-center gap-2">
              <button 
                ...
                className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all ${currentStep === 0 ? 'text-slate-100 bg-slate-50/50' : 'text-slate-400 bg-white border border-slate-100 hover:bg-slate-50 hover:text-slate-900 shadow-sm'}`}
              >
              ...
              <button 
                onClick={handleNext}
                className="px-6 py-3 bg-slate-950 text-white rounded-xl text-[12px] font-black flex items-center gap-2 hover:bg-indigo-600 transition-all shadow-xl shadow-slate-200 active:scale-95"
              >
                <span>{currentStep === TOUR_STEPS.length - 1 ? 'فهمت، ابدأ!' : 'الخطوة التالية'}</span>
                <ChevronLeft size={16} />
              </button>
            </div>
          </div>
