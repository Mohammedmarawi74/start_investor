...
export const LeadersSection: FC<{
  ...
  >
    <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20, direction: 'rtl' }}>
      <div className="sd-bar" style={{ width: 4, height: 44, borderRadius: 4, flexShrink: 0 }} />
      <h2 className="sd-section-title" style={{ margin: 0, fontWeight: 800, color: '#0f172a', letterSpacing: '-0.02em' }}>قادة {title.replace(/^(قطاع|صناعة)\s*/, '')}</h2>
    </div>
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(240px, 1fr))', gap: 12, direction: 'rtl' }}>
      {leaders.map((leader, idx) => (
        <div
          key={idx}
          className="sd-leader-card"
          style={{ position: 'relative', background: '#f8fafc', borderRadius: 16, border: '1px solid #e2e8f0', padding: '16px 16px 16px 40px', transition: 'border-color 0.2s, box-shadow 0.2s, background 0.2s', cursor: 'default' }}
          onMouseEnter={(e) => {
